...
elasticsearch.prototype.setMapping = function(data, limit, offset, callback){
  ...
  // returns a function that will put a given mapping to given url
  var uploadMappingHandler = function(url, mapping, callback) {
    return function uploadMapping(err, response) {
      var payload = {
        url: url,
  ...
  var putMappings = function(mappings, callback) {
    var totalTypes = Object.keys(mappings).length;
    var remainingTypes = totalTypes;

    var doneUploadingMapping = function(err) {
      self.parent.emit('log', 'finished processing a type.');

      if (--remainingTypes === 0) {
        // all types for this index have been uploaded
        callback(err, totalTypes);
      }
    };
    ...
    for(var key in mappings) {
      ...
      var typeUrl = self.baseUrl + '/' + key;
      var typeMapUrl = typeUrl + '/_mapping';

      // callback that will upload given map and then finish the process if all are processed
      var uploadMap = uploadMappingHandler(typeMapUrl, mapping, doneUploadingMapping);

      // only upload map if index exists
      request.put(self.baseUrl, uploadMap);
    }
  };
  ...
  if (self.haveSetMapping === true){
    ...
    var remainingIndexes = totalIndexes;
    var processedTypesTotal = 0;

    // set mappings for all indexes
    //for(var index in data) {
    indexes.forEach(function(index) {
      var mappingsForIndex = data[index].mappings;

      putMappings(mappingsForIndex, function(err, processedTypes) {
        self.parent.emit('log', 'finished processing an index: ' + index);

        processedTypesTotal += processedTypes;

        if (--remainingIndexes === 0) {
          self.haveSetMapping = true;
          callback(err, processedTypesTotal);
        }
      });
    });
  }
